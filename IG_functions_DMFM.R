TS_IG_MultiNormal_DMFM_Poi <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, lambda, k_ini, M_max){
  ## Model: M-1 \sim Poisson(lambda)
  ##        y | mu, Sigma \sim N(y | mu, Sigma) 
  ##        mu \sim N_r(b_0, B_0), Sigma^{-1} | C \sim W(c_0, C), C \sim W(g_0, G_0)
  ##        h_m | Alpha, M \sim IGau(Alpha/M, 1), m=1,...,M
  ##        Alpha \sim F(df1, df2)
  ##        
  ...
  for (iter in 2:MCMC_iteration) {
    ...
    ### update M and M_na ###
    M <- sample_M_TS_IG_Poi(M_max, k, U_n, Alp, lambda, c_rename)
    M_na <- M - k
    ...
    if(M_na > 0){
      S_na <- sapply(1:M_na, function(x) rgig(1, lambda = - 0.5, chi = Alpha^2, psi = 2 * U_n + 1))
...
TS_IG_MultiNormal_DMFM_NB <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, r, p, k_ini, M_max){
  ...
  for (iter in 2:MCMC_iteration) {
    ...
    ### update M_na ###
    psi_u <- exp(log_psi(U_n, Alpha))
    M <- sample_M_TS_IG_NB(M_max, k, U_n, Alp, r, p, c_rename)
    M_na <- M - k
...
TS_IG_MultiNormal_DMFM_NB_fix <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, r, p, k_ini, Alpha, M_max){
...
TS_IG_MultiNormal_DMFM_BNB <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, al_lam, a_pi, b_pi, k_ini, M_max){
  ...
  for (iter in 2:MCMC_iteration) {
    ...
    ### update M and M_na ###
    M <- sample_M_TS_IG_BNB(M_max, k, U_n, Alp, al_lam, a_pi, b_pi, c_rename)
    M_na <- M - k

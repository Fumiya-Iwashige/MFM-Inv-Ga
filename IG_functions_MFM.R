BG_IG_MultiNnormal_MFM_Poi <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, lambda, k_ini){
...
BG_IG_MultiNnormal_MFM_NB <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, a_lambda, b_lambda, k_ini){
  ...
  for (iter in 2:MCMC_iteration) {
    ...
    ### update allocated parameters:S_a, Theta_a and Sigma_a ###
    for (i in 1:k) {
      # SS_a <- rgig(1, lambda = c_table[i] - 0.5, chi = Alpha^2 * 10^2, psi = (2 * U_n + 1)/10^2 )
      # S_a[i] <- SS_a / 10^2
...
BG_IG_MultiNnormal_MFM_NB_fix <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, a_lambda, b_lambda, Alpha, k_ini){
  ## Model: M-1 | lambda \sim Poisson(lambda)
  ##        lambda \sim Gamma(a_lambda, b_lambda)
  ##        y | mu, Sigma \sim N(y | mu, Sigma)
  ##        mu \sim N_r(b_0, B_0), Sigma^{-1} | C \sim W(c_0, C), C \sim W(g_0, G_0)
...
BG_IG_MultiNnormal_MFM_BNB <- function(data, MCMC_iteration, b_0, B_0, c_0, g_0, G_0, a_lambda, a_pi, b_pi, k_ini){
  ## Model: M-1 | lambda \sim Poisson(lambda)
...
BG_IG_UniNormal_MFM <- function(data, MCMC_iteration, m_0, c_0, d_0, D_0, w_0, W_0, a_lambda, b_lambda, Alpha){
  ## Model: M-1 | lambda \sim Poisson(lambda)
  ##        lambda \sim Gamma(a_lambda, b_lambda)
  ##        y | mu, Sigma \sim N(y | mu, Sigma) 
